...
- 音标与读音的来源优先级必须明确：
  1. 先查 `Oxford`
  2. 如果 `Oxford` 没有这个词，再使用本地人工补充
- `Cambridge` 不能作为正式回退源，因为国内用户无法稳定访问
- 能用 `Oxford` 就不要用人工补充
- 人工补充只是兜底方案，不能反过来覆盖本来已经能从正式词典拿到的数据
- 点击单词后的详情弹框，必须展示该词的完整中文释义，完整释义取自 `words.json` 的 `meaning_cn`，不能只显示正文里那个单个义项
- 故事正文里 `word [中文]` 的 `中文` 只显示一个最贴合当前语境的义项；弹框里则显示 `meaning_cn` 的完整释义，两者不要混淆
- 详情弹框必须同时展示英式音标、英式读音、美式音标、美式读音，缺一不可
- 音标展示格式统一为 `/.../`；如果数据源本身没有前后斜杠，页面展示时也要自动补齐
- 英式和美式音标、读音必须分别对应，不能把同一套内容同时当作英式和美式展示
- 音标和读音要尽量来自正式词典来源；如果词典缺失，才允许用人工补充
- 人工补充时，音标必须正确，英式和美式都要有；读音文件必须放在项目本地可访问地址，不能依赖 `translate.google.com` 这类国内不可用外链
- 当前项目里，手工补充词的读音统一放在 `public/audio/manual/uk/` 和 `public/audio/manual/us/` 目录下
- 如果修改了 `words.json` 里的释义、音标或读音，页面接口与弹框展示也必须同步生效，不能出现词表已改但页面还显示旧内容的情况
